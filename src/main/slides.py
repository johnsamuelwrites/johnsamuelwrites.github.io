...
def enumerate_slides(filename):
  ...
  with open(filename,"r") as inputfile:
    for line in inputfile:
       ...
       #Setting up regular expressions
       innersectionline1 = regex1.sub('<section class="slide" id="slide%d">' % count, line)
       innersectionline2 = regex2.sub('<div class="navigation">%d' % count, line)
       innersectionline3 = regex3.sub('<a class="prev" href="#slide%d">' % (count-1), line)
       newsectionline = regex4.sub('<a class="next" href="#slide%d">' % (count+1), line)
