...
class BlogGenerator:
    ...
    def is_recently_updated(modification_time: int) -> bool:
        ...
        return (current_time - modification_time) < BlogGenerator.RECENT_UPDATE_THRESHOLD
    ...
    def extract_article_metadata(filepath: str) -> ArticleMetadata:
        ...
        with open(filepath, 'r', encoding='utf-8') as f:
            ...
            title = BlogGenerator.clean_title(title_tag.text) if title_tag else "Untitled"
            
            # Get timestamps
            creation_time, modification_time = get_first_latest_modification(
                filepath.replace('/mnt/user-data/uploads/', '').replace('/home/claude/', '')
            )
    ...
    def generate_article_html(article: ArticleMetadata, position: int) -> str:
        ...
        if article.is_recently_updated:
            updated_badge = '<span class="updated-badge" title="Recently Updated">🔥</span>'
        
        modified_indicator = ""
        if article.is_updated:
            modified_indicator = f'<span class="modified-date">{article.modification_date_str}</span>'
    ...
    @staticmethod
    def organize_articles_by_year(articles: List[ArticleMetadata]) -> Dict[int, List[ArticleMetadata]]:
    ...
    @staticmethod
    def generate_year_section(year: int, articles: List[ArticleMetadata], start_position: int) -> str:
        """Generate HTML for a year section."""
        article_count = len(articles)
        
        articles_html = ""
        for i, article in enumerate(articles):
            articles_html += BlogGenerator.generate_article_html(article, start_position + i)
    ...
    @staticmethod
    def _generate_multilingual_blog(articles_by_language: Dict[str, List[ArticleMetadata]]) -> None:
        """Generate the main multilingual blog page."""
        # Generate content for each language
        language_content = {}
        
        for lang_code in ['en', 'fr', 'ml', 'pa', 'hi']:
            if lang_code not in articles_by_language or not articles_by_language[lang_code]:
            ...
            for year in years:
                ...
                year_sections += BlogGenerator.generate_year_section(year, year_articles, position)
                position += len(year_articles)
            
            # Wrap in language section
            language_content[lang_code] = f"""
        ...
        content = content.replace('MalayalamArticleList', language_content.get('ml', ''))
    ...
    @staticmethod
    def _generate_multilingual_blog1(articles_by_language: Dict[str, List[ArticleMetadata]]) -> None:
        """Generate the main multilingual blog page."""
        language_names = {
            'en': 'English',
            'fr': 'Français',
    ...
    @staticmethod
    def _generate_language_specific_blogs(articles_by_language: Dict[str, List[ArticleMetadata]]) -> None:
