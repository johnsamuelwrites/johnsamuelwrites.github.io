        ...
        for link in parsed_html.find_all('title'):
          ...
          for lang in ["en", "fr", "hi", "pa", "ml"]:
            if article.startswith(lang):
              articlelist[lang] = articlelist[lang] + "\n<li property='itemListElement' typeof='ListItem'>" + '<meta typeof="ListItem" property="position" content="' + str(count[lang]) + '"/>' + "<a property='item' typeof='WebPage' href='../"+ article + "'>" + "<span property='name'>" + title + "</span></a>" + " <span class='date'>" + datetime.fromtimestamp(time).strftime('%d %B %Y') + "</span></li>" 
